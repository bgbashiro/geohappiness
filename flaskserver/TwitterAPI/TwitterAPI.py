...
# modified request so that it can accept None as a valid
# GET request (there is bug in requests package)
class TwitterAPI(object):
    ...
    def _prepare_url(self, subdomain, path, params=None):
        if subdomain == 'curator':
            url =  '%s://%s.%s/%s/%s.json' % (PROTOCOL,
                                              subdomain,
                                              DOMAIN,
                                              CURATOR_VERSION,
                                              path)
        elif subdomain == 'ads-api':
            url = '%s://%s.%s/%s/%s'      % (PROTOCOL,
                                              subdomain,
                                              DOMAIN,
                                              ADS_VERSION,
                                              path)
    ...
    def request(self, resource, params=None, files=None, method_override=None):
        ...
        with requests.Session() as session:
            ...
            url = self._prepare_url(subdomain, resource, params=params)
